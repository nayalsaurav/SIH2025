...
const QRCodeDisplay: React.FC<QRCodeDisplayProps> = ({ value, size = 160 }) => {
  ...
  <QRCodeSVG value={value} height={size} width={size} fgColor="#4f46e5" bgColor="transparent" />
      <span className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">Batch ID: {value}</span>
    </div>
  );
};
