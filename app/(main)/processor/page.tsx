...
import {
  Factory, Package, Thermometer, Droplets,
} from 'lucide-react';
...
function ProcessorDashboard() {
  ...
  const validatedCollections = collections.filter((collection) =>
    qualityTests.some(
      (test) =>
        test.collectionEventId === collection.id && test.validated
    )
  );
  ...
  const handleCreateProduct = () => {
    ...
    const relatedTests = relatedCollections
      .map((collection) =>
        qualityTests.find(
          (test) => test.collectionEventId === collection.id
        )
      )
    ...
    >
      ...
      >
        ...
        <div>
          ...
          <p className="text-zinc-700 dark:text-zinc-300">{processingData.processorName} - ID: {processingData.processorId}</p>
        </div>
      </motion.div>

      <div className="grid lg:grid-cols-2 gap-10 mb-10">
        {/* Add Step Form */}
        <motion.div className={`${glass} rounded-2xl p-8 shadow-xl`} variants={fadeUp} custom={1}>
          ...
          <form onSubmit={handleAddStep} className="space-y-5">
            ...
            <div>
              <h3 className="text-xs font-semibold text-purple-700 mb-2">Processing Conditions</h3>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setProcessingData((prev) => ({
                        ...prev,
                        conditions: { ...prev.conditions, temperature: parseInt(e.target.value) },
                      }))
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setProcessingData((prev) => ({
                        ...prev,
                        conditions: { ...prev.conditions, humidity: parseInt(e.target.value) },
                      }))
                    }
                    className="w-full px-2 py-1 text-sm border border-zinc-300 rounded focus:ring-2 focus:ring-purple-400 focus:border-purple-600 dark:bg-zinc-900/70 dark:border-zinc-600 dark:text-white"
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setProcessingData((prev) => ({
                        ...prev,
                        conditions: { ...prev.conditions, duration: parseInt(e.target.value) },
                      }))
                    }
                    className="w-full px-2 py-1 text-sm border border-zinc-300 rounded focus:ring-2 focus:ring-purple-400 focus:border-purple-600 dark:bg-zinc-900/70 dark:border-zinc-600 dark:text-white"
                  />
        ...
        <motion.div className={`${glass} rounded-2xl p-8 shadow-xl`} variants={fadeUp} custom={2}>
          <h2 className="text-2xl font-semibold text-zinc-900 dark:text-white mb-5">Available Validated Batches</h2>
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-semibold text-purple-900 dark:text-purple-300">{collection.species}</h3>
      ...
      {/* Recent Steps */}
      <motion.div className={`${glass} rounded-2xl p-8 shadow-xl mt-8`} variants={fadeUp} custom={3}>
        <h2 className="text-2xl font-semibold text-zinc-900 dark:text-white mb-5">Recent Processing Steps</h2>
          ...
          {processingSteps.slice(-4).map((step, idx) => (
            ...
            >
              <div className="flex justify-between items-center mb-2">
                <h3 className="font-semibold text-fuchsia-900 dark:text-fuchsia-200">{step.step}</h3>
              ...
              <div className="text-xs text-zinc-700 dark:text-zinc-200">
                <p>Input: <span className="font-mono">{step.inputBatch}</span></p>
                <p>Output: <span className="font-mono">{step.outputBatch}</span></p>
                <p>Conditions: {step.conditions.temperature}°C, {step.conditions.humidity}% RH</p>
                <p>Duration: {step.conditions.duration} hours</p>
                <p>Processed: <span className="font-mono">{formatDate(step.timestamp)}</span></p>
              </div>
            </motion.div>
          ))}
        </div>
